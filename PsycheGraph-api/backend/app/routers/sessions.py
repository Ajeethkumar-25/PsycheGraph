...
#         soap_notes=processed_data.get("summary", "") # Fixed: use soap_notes column
#     )
...
from .. import models, schemas, dependencies, database
from ..services import audio
from ..services.audio import get_supported_languages
import shutil
import os
import uuid
from datetime import datetime, timezone
...
async def list_supported_languages():
    """
    Returns all supported languages for transcription.
    Frontend uses this to show a language dropdown.
    """
    ...
    return {
        ...
        "note": "Pass the 'code' value as the 'language' field when creating a session. Leave empty for auto-detect."
    }


# ─── Create Session with Language Support ──────────────────────────────
@router.post("/", response_model=schemas.SessionOut)
async def create_session(
    patient_id: int = Form(...),
    file: UploadFile = File(...),
    language: Optional[str] = Form(None),   # ← e.g. 'en', 'ta', 'hi' — None = auto-detect
    ...
    language options (pass the code):
    ...
    Leave language empty to auto-detect.
    """
    ...
    # ✅ Process audio with language
    try:
        processed_data = await audio.process_audio_file(file, language=language)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Audio processing failed: {str(e)}")

    detected_language = processed_data.get("language", "Unknown")
    ...
    new_session = models.Session(
        ...
        transcript=processed_data.get("english_translation", ""),
        summary=processed_data.get("summary", ""),
        soap_notes=processed_data.get("summary", "")
    )
